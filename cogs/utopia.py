...
class utopiaCog(commands.Cog):
    ...
    async def leaderboard(self, ctx):
        ...
        dsorted = sorted(data.keys(), key=lambda x: (
            data[x]['reputation']), reverse=True)
        number = 1
        for i in dsorted:
    ...
    async def give(self, ctx, member: typing.Union[discord.Member, int], *, reason):
        ...
        if member == ctx.author:
            raise commands.BadArgument(
                'Woooooooooow, yeah I\'m not gonna do that bud.')
        with open('rep.json', 'r') as f:
            data = json.load(f)
        try:
    ...
    async def nominate(self, ctx, message: discord.Message):
        ...
        if len(message.attachments):
            for i in message.attachments:
                finmsg += f'\n{i.url}'
        nominees[f'{str(finmsg)}∫√∆{str(message.author)}∫√∆{str(message.created_at)}'] = str(
            message.id)
